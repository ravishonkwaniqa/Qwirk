...
public class NewLogForEachRunFileAppender extends FileAppender {
	...
	//for every time create new log in new file
	 public NewLogForEachRunFileAppender() {
	    ...
	    if (fileName != null) {
	    	final String timeStamp = new SimpleDateFormat("yyyy.MM.dd.HH.mm.ss").format(new Date());//time stamp
	    	...
	    	if (dotIndex != -1) {
	    		...
	    		newFileName = fileName.substring(0, dotIndex)  + HIPHEN+ timeStamp +  DOT +  fileName.substring(dotIndex +  1);
	    	} else {
